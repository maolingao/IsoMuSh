...
#define LastUpdate "25-Nov-2003"						/* The last update
														   of the program.	*/
...
extern void     MarchAlg(struct Vertex  *V, int Vnum, double *srcval, double *srcnum); 
...
bool phrase_args(int nlhs,			/* Number of expected output arrays */
...
//	*xCoordinates = mxGetData(prhs[1]);					/* Set the x and y coordinates arrays to the place in the data of		*/
	...
	SourceVal = (double *)mxGetData(prhs[1]);
	...
	*yCoordinates = NULL;
	...
	if(nrhs == 3 || nrhs == 2){						/* If the first array is a surface array.								*/
		*CoordinatesData = (const double *)mxGetData(prhs[0]);			/* The coordinates are the data of the surface array.					*/

		if(nrhs == 3){
			DimensionsArray = (const int *)mxGetData(prhs[2]);		/* Get the sizes of the surface											*/
...
//	printf("\nELAPSED TIME: %f ms\n", tm);	
	...
	plhs[0] = mxCreateDoubleMatrix(Vnum, 1, mxREAL);
	plhs[1] = mxCreateDoubleMatrix(Vnum, 1, mxREAL);
	DistancesData = (double *)mxGetData(plhs[0]);
	srclabel =  (double *)mxGetData(plhs[1]);
	for(i = 0; i < Vnum; i++){	/* Fill up the result matrix with data.					*/
		*(DistancesData++) = V[i].U;
		*(srclabel++) = V[i].source_num;
	}
